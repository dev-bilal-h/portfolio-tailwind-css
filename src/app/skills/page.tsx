...
const skillsData = [
  { name: 'HTML5', icon: <FaHtml5 className="text-orange-500 text-3xl sm:text-4xl" />, level: 92 },
  { name: 'Next.js', icon: <SiNextdotjs className="text-slate-900 text-3xl sm:text-4xl" />, level: 88 },
  { name: 'CSS3', icon: <FaCss3Alt className="text-sky-300 text-3xl sm:text-4xl" />, level: 85 },
...
const Skills = () => {
  return (
    //skill section
    <div className="min-h-screen flex items-center justify-center bg-black text-white px-2 sm:px-8">
      <div className="w-full max-w-4xl p-2 sm:p-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 mt-16 mb-16 animate-slideInFromBottom">
          {skillsData.map((skill, index) => (
            <div
              key={index}
              className="flex items-center p-4 bg-sky-800 rounded-lg border-sky-400 border-2 hover:bg-sky-700 hover:shadow-lg transition duration-700 hover:scale-105"
            >
              ...
              <div className="ml-auto w-1/2 mr-0 sm:mr-4">
                <div className="bg-yellow-50 rounded-full h-1.5 sm:h-2 mt-2 relative">
                  <div
                    className="absolute top-0 left-0 h-1.5 sm:h-2 rounded-full bg-sky-300"
